...
handle_string(struct parser *p, const char **str, size_t *str_len, bool *allocated)
{
	...
	len_estimate += 1;
	size_t len = 0;
	char *data = calloc(len_estimate + 1, sizeof(char));
	for (off = 0; p->pos + off < p->size; off++) {
		char c = p->input[p->pos + off];
		if (c == '"')
			break;
...
cson_parse(char *data, int64_t size)
{
	...
	struct cson *
cson_parse_file(const char *file_path, const char **freewhendone)
{
	FILE *f = fopen(file_path, "r");
	if (!f || fseek(f, 0, SEEK_END) == -1)
		return NULL;

	size_t fsize = ftell(f);
	char *data = (char *)calloc(fsize, 1);
	if (fseek(f, 0, SEEK_SET) == 0 && fread(data, fsize, 1, f) == 1) {
		*freewhendone = data;
		fclose(f);
		return cson_parse(data, fsize);
	}
	int err = errno;
	fclose(f);
	free(data);
